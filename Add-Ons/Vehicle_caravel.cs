...
datablock ExplosionData(caravelExplosion: jeepExplosion)
{
   debrisNum = 0;
};

datablock ProjectileData(caravelExplosionProjectile)
{
   directDamage        = 0;
   radiusDamage        = 0;
   damageRadius        = 0;
   explosion           = caravelExplosion;
...
datablock ExplosionData(caravelFinalExplosion)
{
     //explosionShape = "";
   lifeTimeMS = 150;
...
datablock ProjectileData(caravelFinalExplosionProjectile)
{
   directDamage        = 0;
   radiusDamage        = 0;
   damageRadius        = 0;
   explosion           = caravelFinalExplosion;
...
//---------------------------------------------------------------------------------------
// Drone Definition
datablock FlyingVehicleData(caravel)
{
	spawnOffset						= "0 0 2";
	emap							= true;
	category						= "Vehicles";
    shapeFile                       = "~/shapes/caravelfinal.dts";
	multipassenger					= True;
	computeCRC						= true;
   ...
   stationaryThreshold = 0;
    ...
    cameraOffset = 3;        // Vertical offset from camera mount point
	cameraMaxDist					= 28;
	cameraLag						= 0.1;
	...
	maneuveringForce				= 13000;      // Horizontal jets (W,S,D,A key thrust)
	...
	// Turbo Jet
	jetForce						= 13000;      // Afterburner thrust (this is in addition to normal thrust)
...
function caravel::onEnterLiquid(%this, %obj, %coverage, %type)
{
	cancel(%obj.floatSchedule);
}
function caravel::onLeaveLiquid(%this, %obj, %type)
{
	%obj.floatSchedule = schedule(10,0,lowerboat,%obj);
}
function caravel::onImpact(%this, %obj, %collidedObject, %vec, %vecLen)
{
	%obj.setvelocity("0 0 0");
}
function caravel::onDamage(%this, %obj, %delta)
{
...
function caravel::onAdd(%this,%obj)
{		 
